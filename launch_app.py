...
TERMINAL_PORT = 8000
WRAPPER_PORT  = 8001
WRAPPER_DIR   = "/tmp/politicai-wrapper"
...
<head>
  ...
  <title>PoliticAI — Análise Política</title>
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    :root {{
      ...
      --surface:     #090b10;
      --rail:        #0e1118;
      --line:        rgba(100,140,220,.10);
      --blue:        #5b8cdb;
      --blue-dim:    rgba(91,140,219,.12);
      --blue-glow:   rgba(91,140,219,.25);
      --green:       #6dbf7e;
    ...
    .logo-politic {{ font-family:"Syne",sans-serif; font-weight:800; font-size:17px;
    ...
    .btn-drive  {{ color:var(--blue);  background:var(--blue-dim);
                   border-color:rgba(91,140,219,.25); }}
    .btn-drive:hover  {{ background:var(--blue-glow); border-color:rgba(91,140,219,.5); }}
    ...
    .footer-brand {{
      ...
      font-size: 11px; color: var(--blue); margin-right: 14px;
    }}
    ...
    .btn-provider {{
      ...
      border-radius: var(--radius); cursor: pointer;
      border: 1px solid rgba(91,140,219,.18);
      color: rgba(91,140,219,.55);
    ...
    .btn-provider:hover {{
      ...
      color: var(--blue);
      border-color: rgba(91,140,219,.4);
    }}
    ...
    #modal {{
      background: #0e1320; border: 1px solid rgba(91,140,219,.15);
      border-radius: 8px; padding: 24px; width: 400px; max-width: 90vw;
      box-shadow: 0 24px 64px rgba(0,0,0,.7);
    }}
    .modal-title {{
      font-family: "Syne", sans-serif; font-weight: 800;
      font-size: 15px; color: var(--blue); margin-bottom: 16px;
    }}
    .backup-list {{
    ...
    .backup-item:hover {{ background: rgba(91,140,219,.05); color: var(--ink); }}
    ...
    .modal-close:hover {{ background: rgba(91,140,219,.08); color: var(--blue); }}
...
<body>
  ...
  <div id="topbar">
    <div class="logo">
      <span class="logo-politic">Politic</span><span class="logo-ai">AI</span>
      <span class="logo-tag">v1.0</span>
      <span class="logo-brand">· Análise Política</span>
    </div>
  ...
  <div id="footer">
    <span class="footer-brand">🏛️ PoliticAI</span>
    <span class="footer-sep"></span>
    <span style="color:var(--ink-muted);font-size:10.5px;">Análise Política Baseada em Evidências</span>
  ...
  <div id="provider-overlay" onclick="if(event.target===this)closeProvider()">
    <div id="provider-modal">
      <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:15px;
                  color:var(--blue);margin-bottom:16px;">🔌 Conectar Provedor de IA</div>
      ...
      <div id="prov-step2" style="display:none;">
        ...
        <input id="prov-key-input" type="password"
          ...
          onfocus="this.style.borderColor='rgba(91,140,219,.4)'"
        ...
        <div style="display:flex;gap:8px;">
          ...
          <button onclick="confirmProvider()" style="
            flex:1;padding:9px;background:var(--blue-dim);
            border:1px solid rgba(91,140,219,.3);
    ...
    function connectProvider() {{
      ...
      grid.innerHTML = PROVIDERS.map(p => `
        <button onclick="selectProvider('${{p.id}}')" style="
          ...
          onmouseover="this.style.background='var(--blue-dim)';this.style.color='var(--blue)';this.style.borderColor='rgba(91,140,219,.35)'"
...
def start_wrapper_server():
    _politicai_drive = "/content/drive/My Drive/PoliticAI"
    _base = _politicai_drive if os.path.isdir(_politicai_drive) else _folder_path
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if p == "/api/apikey":
                ...
                try:
                    marker = f"# politicai-key-{provider}"
...
<style>
@keyframes glow {
  0%,100% { box-shadow: 0 0 20px rgba(91,140,219,0.3); }
  50%      { box-shadow: 0 0 40px rgba(91,140,219,0.6); }
}
...
.ready-badge {
  ...
  background:rgba(91,140,219,0.10);border:2px solid rgba(91,140,219,0.35);
  border-radius:12px;animation:glow 2s ease-in-out infinite;
}
.ready-icon { font-size:28px; }
.ready-text { font-family:'DM Mono',monospace;font-size:18px;font-weight:600;color:#5b8cdb; }
</style>
<div class="ready-container">
  <div class="ready-badge">
    <span class="ready-icon">🏛️</span>
    <span class="ready-text">PoliticAI pronto!</span>
  </div>
</div>"""))
...
@keyframes pulse-glow {{
  0%,100% {{ box-shadow:0 0 20px rgba(91,140,219,0.3),0 4px 12px rgba(0,0,0,0.3); }}
  50%      {{ box-shadow:0 0 40px rgba(91,140,219,0.6),0 6px 20px rgba(0,0,0,0.4); }}
}}
...
.pa-launch {{
  ...
  background:linear-gradient(135deg,#2a4a8a 0%,#5b8cdb 50%,#1a3060 100%);
...
<div class="btn-container">
  <a href="{banner_url}" target="_blank" class="pa-launch">
    ...
    <span class="btn-text">
      <span class="btn-main">ABRIR O POLITICAI</span>
      <span class="btn-sub">Análise Política Baseada em Evidências</span>
    </span>
